...
export default function EditLadderForm({ onSubmit, currentValue, saving }) {
  ...
  const html = watch('rule');
  function onChange(e) {
    setValue('rule', e.target.value)
  }
          ...
          <div className="flex flex-wrap">
            <div className="w-full lg:w-12/12 sm:px-4">
              <div className="relative w-full mb-3 ">
                ...
                <DefaultEditor value={html} onChange={onChange} className="border px-3 py-3 placeholder-gray-300 text-gray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150" />
              </div>
            </div>
          </div>
