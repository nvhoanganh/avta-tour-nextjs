...
export default function PlayersPicker({ register, selectedPlayerNumber, filter, competition, otherPlayer, players, setValue, playStyleFilter, playerStyleFilterName, filterName, showSelect }) {
  const filteredPlayers = getPlayers(players, 'Point', filter, competition.maxPoint + (competition.allowMaxPointOverTheLimit || 0) - (otherPlayer?.avtaPoint || 0), playStyleFilter);
  ...
  return (
    <>
      ...
      <div className="text-gray-400 text-sm italic text-center">{filteredPlayers.length} available players with point less than {competition.maxPoint + (competition.allowMaxPointOverTheLimit || 0) - (otherPlayer?.avtaPoint || 0)}</div>
