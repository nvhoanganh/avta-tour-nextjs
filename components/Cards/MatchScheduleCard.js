          ...
          <div className='w-full lg:w-6/12 xl:w-3/12'>
            {schedule[court].map((match) => (
              <div key={match.id} className="relative flex min-w-0 break-words  bg-white rounded mb-3 xl:mb-0 shadow-lg">
                <div className="flex items-center pl-3 font-bold tex-xl">{ match.group}</div>
                <div className="flex-auto py-2">
                  <div className="flex flex-wrap">
                    <div className="relative w-auto pl-1 flex-initial flex items-center">
                      <TeamAvatar team={match.between[0]} />
                      <span className="ml-2 text-lg">{match.between[0].players[0]?.nickName} + {match.between[0].players[1]?.nickName}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap">
                    <div className="relative w-auto pl-1 flex-initial flex items-center">
                      ...
                      <span className="ml-2 text-lg">{match.between[1].players[0]?.nickName} + {match.between[1].players[1]?.nickName}</span>
                    </div>
                  </div>
