...
export async function getPlayersNotInContentful() {
  initFirebase();
  const db = getFirestore();
  const data = await db.collection("users").where("notInContentful", "==", true).get();
  const users = data.docs;
  return users.map(element => {
...
export const CheckAndUpdateClub = async (players) => {
  ...
  clubs.forEach(async club => {
    ...
    if (foundList.length === 1 && foundList[0].playersCount !== playersCount) {
      // check make sure count is right
      console.log(`need to update players count for club ${club} to ${playersCount}, current count is ${foundList[0].playersCount}`);
      await updateClub({ ...foundList[0], playersCount });
