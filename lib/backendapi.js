...
export async function getGroupDetails(id) {
  initFirebase();
  const db = getFirestore();
  const data = await db.collection("groups").doc(id).get();
  if (data.exists) {
    // get players
    const players = await db.collection("groups").doc(id).collection("players").get();
    const scores = await db.collection("groups").doc(id).collection("scores").get();

    const d = data.data();
    return {
      ...d,
      id: data.id,
      createdOn: d.createdOn.toDate().toISOString(),
      startDate: d.startDate?.toDate()?.toISOString() || null,
      players: getItemsAsArray(players),
      scores: getItemsAsArray(scores)
    };
  }
  return null;
}