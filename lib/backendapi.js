...
export async function geCompResults(compId) {
  console.log('getting match result for', compId);
  initFirebase();
  const db = getFirestore();
  const data = await db.collection("competition_results").where("competitionId", "==", compId).get();
