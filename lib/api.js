...
export const getGroupStageStanding = (data, groupAllocations) => {
  ...
  var result = Object.keys(groupedbyGroup).reduce((pre, curr) => {
    ...
    matchesInGroup.forEach(match => {
      // check winners
      let currentTeam = resultForGroup[match.winners.name]

      if (!resultForGroup[match.winners.name]) {
        resultForGroup[match.winners.name] = {
          win: 1,
          name: match.winners.name,
          lost: 0,
          gameWin: 6,
          teamId: match.winnerTeamId,
          gameLost: match.gameWonByLoser,
          players: match.winners.players
        };
      } else {
        resultForGroup[match.winners.name] = {
          ...currentTeam,
      ...
      currentTeam = resultForGroup[match.winners.name]
      resultForGroup[match.winners.name] = {
        ...currentTeam,
      ...
      // check losers
      currentTeam = resultForGroup[match.losers.name]

      if (!resultForGroup[match.losers.name]) {
        resultForGroup[match.losers.name] = {
          win: 0,
          name: match.losers.name,
          lost: 1,
          teamId: match.loserTeamId,
          gameWin: match.gameWonByLoser,
          gameLost: 6,
          players: match.losers.players
        };
      } else {
        resultForGroup[match.losers.name] = {
          ...currentTeam,
  ...
  const final = Object.keys(groupAllocations).reduce((pre, cur) => {
    const teams = groupAllocations[cur].map((teamAlloc) => {
      const result = allresults.find(matchResult => matchResult.teamId === teamAlloc.sys.id);
      return {
        ...teamAlloc,
        win: 0,
        lost: 0,
...
export const GetLadderStanding = (matches, allUsers) => {
  var ranking = {};
  ...
  matches.forEach(match => {
    ...
    winners.forEach(k => {
      ...
      const fuser = allUsers.find(u => u.uid === match[`winnerUser${k}`].uid);
      ...
      if (!player) {
        ranking[match[`winner${k}`]] = {
          ...
          player: { ...match[`winnerUser${k}`], ...fuser }
        };
      } else {
        ranking[match[`winner${k}`]] = {
          ...player,
    ...
    losers.forEach(k => {
      ...
      const fuser = allUsers.find(u => u.uid === match[`loserUser${k}`].uid);
      if (!player) {
        ranking[match[`loser${k}`]] = {
          ...
          player: { ...match[`loserUser${k}`], ...fuser }
        };
      } else {
        ranking[match[`loser${k}`]] = {
          ...player,
