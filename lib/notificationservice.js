...
export async function sendEmailNewLadderResult(user, data) {
  const idtoken = await user.getIdToken();
  const body =
    `${data.submittedBy} just submitted the following score for ${data.ladderName} ladder:\n\n
...
export async function sendEmailDeleteLadderResult(user, data) {
  const idtoken = await user.getIdToken();
  const body =
    `${data.submittedBy} just deleted the following score for ${data.ladderName} ladder:\n
  - ${data.time}: ${data.winnerTeam} defeated ${data.losingTeam} ${data.score}\n\n
...
`;
  ...
  const result = await fetch(
    `/api/sendemail`,
    {
      method: 'POST',
      body: JSON.stringify({
        subject: `[${data.ladderName}] - ${data.submittedBy} just deleted a match !!!`,
        from: 'noreply@avtatour.com',
        to: [...data.toAddresses, 'nvhoanganh1909@gmail.com'],
        text: body,
      }),
