import { compose, init, last, groupBy, sort, descend, splitEvery, uniq, intersection, flatten, sortBy, prop, pipe, countBy, identity, F, forEach } from 'ramda';
...
export const GamePerGroupStage = 6;
...
export const getSortLabel = (sortString) => {
  if (sortString === 'compsPlayed') {
    return 'Comp Played';
  }

  if (sortString === 'monthsSinceLastComp') {
    return 'Last Played';
  }
  return sortString;
}
...
export const getPlayers = (allPlayers, sortBy, filter, maxPoint, playerStyle) => {
  ...
  const sorted = allPlayers.sort((a, b) => {
    if (sortBy === 'Point') {
      return getDiffNumber(a.avtaPoint, b.avtaPoint);
    }
    if (sortBy === 'Club') {
    ...
    if (sortBy === 'compsPlayed') {
      return getDiffNumber(a.compsPlayed, b.compsPlayed);
    }

    if (sortBy === 'monthsSinceLastComp') {
      return getDiffNumber(b.monthsSinceLastComp || 99999, a.monthsSinceLastComp || 99999);
    }
  });
...
export const getFilteredLadderMatches = (allMatches, filter) => {
  if (filter?.trim()) {
    return allMatches.filter(x => {
      let allNames =
        [x.winnerUser1?.displayName || x.winnerUser1?.fullName,
        x.winnerUser2?.displayName || x.winnerUser2?.fullName,
        x.loserUser1?.displayName || x.loserUser1?.fullName,
        x.loserUser2?.displayName || x.loserUser2?.fullName,
        x.winnerUser1?.nickName, x.winnerUser2?.nickName,
        x.loserUser1?.nickName, x.loserUser2?.nickName].filter(x => !!x).map(x => Diacritics.clean(x.toLocaleLowerCase()).split(' '));
  
      allNames = uniq(flatten(allNames));
  
      const cleanedFilter = Diacritics.clean(filter.toLowerCase().trim()).split(' ');
      const intersec = intersection(cleanedFilter, allNames);
      
      if (cleanedFilter.length > 1 && (intersec.length === 0 || intersec.length < cleanedFilter.length)) {
        const allExceptLast = init(cleanedFilter);
        const lastN = last(cleanedFilter);
        const newIntersec = intersection(allExceptLast, allNames);
        return newIntersec.length >= allExceptLast.length && allNames.filter(x => x.indexOf(lastN) >= 0).length > 0;
      }
  
      if (cleanedFilter.length === 1 && intersec.length === 0) {
        return allNames.filter(x => x.startsWith(cleanedFilter[0])).length > 0;
      }
  
      if (cleanedFilter.length === 1) return intersec.length > 0;
      return intersec.length >= cleanedFilter.length;
    });
  }
...
export const getCompGroupsV2 = (allTeams, teamsInEachGroup, numberOfGroups) => {
  ...
  const groupBeforLast = groups[groups.length - 2];

  if (groupBeforLast.length === teamsInEachGroup && lastGroup.length === teamsInEachGroup - 2) {
    // last group has 2 less than the required, then just move the last player of group before last to it then we're good
    const lastTeamOfGroupBeforeLast = last(groupBeforLast);
    lastGroup.push(lastTeamOfGroupBeforeLast);
    groups[groups.length - 2] = groupBeforLast.slice(0, groupBeforLast.length - 1);
  } else if (allTeams.length % numberOfGroups > 0) {
    // not even split, need to distribute the member of the last group
    const lastGroup = last(groups);
    const teamsBeforeLast = groups.slice(0, groups.length - 1);
    for (let index = 0; index < lastGroup.length; index++) {
      // move the players of the last group to each starting group
      teamsBeforeLast[index].push(lastGroup[index])
    }
...
export function getWinningScoreForComp(result) {
  if (result.stage === 'Group Stage') return GamePerGroupStage;
