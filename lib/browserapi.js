import { compose, groupBy, sort, descend, last, splitEvery, uniq, intersection, flatten, sortBy, prop, pipe, countBy, identity, F, forEach } from 'ramda';
...
export const getFilteredLadderMatches = (allMatches, filter) => {
  if (filter?.trim()) {
    return allMatches.filter(x => {
      ...
      if (intersec.length === 0 && cleanedFilter.length === 1) {
        // use start with if there is only 1 name
